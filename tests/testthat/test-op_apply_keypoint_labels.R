...
test_that("op_apply_keypoint_labels renames columns based on region", {
  ...
  expect_true(
    any(sample_data$region == "body"),
    info = "The sample data does not include 'body' region."
  )
  ...
  expect_true(
    any(body_labels %in% names(renamed_data)),
    info = "Body keypoint labels were not applied correctly."
  )
  ...
  expect_false(
    any(face_labels %in% names(renamed_data)),
    info = "Face keypoint labels should not be present in the renamed data."
  )
})
